...
interpretOne (Command var (Literal val)) = do
        ...
        -- where help_liter [] var val = [(var, val)]
        --       help_liter (x:xs) var val | (fst x) == var = (  [(var, val)] ++ xs  ) 
        --                                 | otherwise = [x] ++ help_liter xs var val 
interpretOne (Command var (VariableReference val)) = do
            temp <- get
            put $ change_val temp var (find_real_val temp val)

            where find_real_val (x:xs) val  | fst x == val = snd x
                                            | otherwise = find_real_val xs val
                --   help_var_refer [] var val = [(var, val)]
                --   help_var_refer (x:xs) var val | (fst x) == var = (  [(var, val)] ++ xs  ) 
                --                             | otherwise = [x] ++ help_var_refer xs var val 
